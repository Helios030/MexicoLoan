...
//        private var logPath = BaseApplication.sContext.filesDir.absolutePath+"log" // log过滤器
...
//            val fileName: String = "$logPath/log_test.log" //log日志名，使用时间命名，保证不重复
            val fileName: String = "/log_test.log" //log日志名，使用时间命名，保证不重复
            val log: String = "  $type $tag $msg\n" //log日志内容，可以自行定制
          d("save log  $log")
            //如果父路径不存在
            val file = BaseApplication.sContext.filesDir
            if (!file.exists()) {
                file.mkdirs() //创建父路径
            }
